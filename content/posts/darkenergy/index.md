...
date: 2025-11-20T18:47:40-06:00
...
Throughout my senior year at UT, I've been taking astronomogy and cosmology courses out of personal interest. As you may know, dark energy is supposedly responsible for the accelerated expansion of the universe. In this blog post, I will discuss my team's research on the temporal evolution of dark energy through analysis of the DESI (Dark Energy Spectroscopic Instrument) and DES (Dark Energy Survey) datasets. *Our code can be found [here](https://github.com/devddesai/temporal_dark_energy). There are many interactive visuals on this notebook to play around with.*
...
In the DESI dataset, we can find `chain1`, `chain2`, `chain3`, `chain4` files, which contain  Monte Carlo simulations of the universe with different parametrizations and their fidelities with respect to current astronomical surveys. In this, they tested the CPL parametrizations, and had a respective $\chi^2$ value, allowing us to see which parametriztion best fits the universe. 

We plotted a heatmap of the data to see this.

{{< figure src="./images/heatmap.png" width="600px" align="center" caption="A heatmap showing different parametrizations of ($w_0, w_a$), and their $\chi^2$ values. We have placed the $\Lambda$CDM's location as well as the best fit location with a circle.">}} 
...
We turned to the DES dataset, where we found Distance Modulus, $\mu$, of Type Ia supernovae and their associated redshift values, $z$. Just for fun, I created an animation of how the expansion of the universe would look according to the distance modulus and redshift values we saw from the DES dataset:

{{< figure src="./images/expansion.gif" width="800px" align="center">}} 

So how can we test the CPL parametrization with this data? Well, $\mu$ and $z$ follow a certain relationship according to the parametrization of the equation of state, $w$. If we use the CPL parametrization, this yields the following relation:

$$\mu(z) = 5log_{10}[\frac{(1+z)\frac{c}{H_{0}} \int_{0}^{z} \frac{dz'}{E(z')} \, d(z') }{10 pc}] + M$$

where $$ E(z) = [\Omega_{m}(1+z)^{3} + \Omega_{r}(1+z)^{4} + \Omega_{\Lambda}e^{3 \int_{0}^{z} \frac{1+w(z')}{1+z} \, d(z') }]^{1/2} $$

From here, we can run a regression to find which parametrization best fits our $(z, \mu)$ data. Things get a bit hairy as this is not a linear relationship, and has an integral in it. Hence, I decided to utilize particle swarm optimization, as the parameter space isn't too large. 

<!-- Here is an interactive visual allowing us to visualize the parameter space and its behavior on $\mu$. 

**insert visual** -->

>**Particle Swarm Optimization**, or PSO, is an optimization algorithm which is useful when dealing with non-linear, non-differentiable functions or complex parameter spaces where gradient-based methods may fail. It initializes some $n$ particles, or parametrization candidates, at random locations with random velocities in the parameter space. It calculates the loss function for each particle, and decides who the best candidate is. At the next step, the particles move according to their velocity, and the velocity is then updated with a weighted sum of the particle's previous momentum and the velocity to reach best candidate's location. After a series of steps, the particles should converge to the optimal candidate.
...
{{< figure src="./images/pso_animation.gif" width="800px" align="center">}} 

From this, we derive that the optimal parametrization of $(w_0, w_a)$ is ~(-0.75, -1), leading us to believe that the CPL parametrization is more accurate at describing the nature of dark energy than the $\Lambda$CDM model. Let's see how this parametrization changes the relation between $\mu$ and $z$.

{{< figure src="./images/finalplot2.png" width="800px" align="center">}} 

Interesting, the CPL and $\Lambda$CDM models look to be basically the same upon visual inspection. I ran some rigourous statistical tests and found that the p-value was ~$10^{-4}$, indicating that there is high confidence that our CPL parametrization fits the data better!

{{< figure src="./images/finalplot1.png" width="800px" align="center" caption="A heatmap showing different parametrizations of ($w_0, w_a$), and their $\chi^2$ values. We have placed the $\Lambda$CDM's location as well as the best fit location with a circle.">}} 


# Discussion

The nature of dark energy is an open question in cosmology. Currently, I am working with Gautam and Professor Mitra at UT's CS department to extend this project and plan to publish in the AAS Conference!